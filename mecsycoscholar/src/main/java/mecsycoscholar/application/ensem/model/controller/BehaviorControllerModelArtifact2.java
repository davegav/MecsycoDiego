 ...
 *
 */
public class BehaviorControllerModelArtifact2 extends ModelArtifact {
	...
	//private String[] simInputPortNames;
	//private double[] simInputs;
	...
	public BehaviorControllerModelArtifact2(String aModelId) {
		super(aModelId);
	}
	...
	public BehaviorControllerModelArtifact2(String aModelId,
		...
		this.inputPortNames = new String[] {"Temp1", "Temp2", "Temp3", "Pow1_out", "Pow2_out", "Pow3_out", "Occup","TempMinIn", "TempMaxIn", "Iteration"};
		this.outputPortNames = new String[] {"TempMinOut", "TempMaxOut", "Comfort", "Consum"};
		...
		this.Outputs = new double[] {((Number) parameters.get("TempMin")).doubleValue(),
				...
				((Number) parameters.get("Comfort")).doubleValue(),
	...
	public void processInternalEvent(double aTime) {
		if (this.inputs[9] > this.InternalIteration) {
			this.InternalIteration = (int) this.inputs[9];
			this.Outputs[2]=0.0;
			this.Outputs[3]=0.0;
		...
		this.Outputs[2] += (Math.abs(OptTemp- this.inputs[0]) + Math.abs(OptTemp- this.inputs[1]) + Math.abs(OptTemp- this.inputs[2]))* this.inputs[6]; 

		// Consum = Pow1_out + Pow2_out + Pow3_out
		this.Outputs[3] += Math.abs(this.inputs[3]) + Math.abs(this.inputs[4]) + Math.abs(this.inputs[5]);
