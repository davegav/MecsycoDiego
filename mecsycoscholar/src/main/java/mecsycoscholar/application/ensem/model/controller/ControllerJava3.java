...
public class ControllerJava3 {
	...
	public ControllerJava3(String[] inputPortNames, String[] outputPortNames, double period,
	...
	public void updateValue(String inputPortName, double value) {
		...
		if (!valueUpdated && !inputPortName.equals("Occup")) {   //TODO temporal fix, do I need to update this here?
	...
	public void computeOrders(double time) {
		...
		// example
		for (int i = 0; i < 3; i++) {
			if (temp[i] < this.minTemp && pow[i] == 0) // We fall below the wanted temperature
			{
				pow[i] = -maxPow[i];
			} else if (temp[i] < this.minTemp && pow[i] > 0) { // Conditioner is on and we fall below the wanted
																// temperature
				pow[i] = 0;
			} else if (temp[i] > this.maxTemp && pow[i] == 0) {
				pow[i] = maxPow[i];
			} else if (temp[i] > this.maxTemp && pow[i] < 0) { // Heater is on, and we are over the wanted temperature
				pow[i] = 0;
			} 
		}
	}
