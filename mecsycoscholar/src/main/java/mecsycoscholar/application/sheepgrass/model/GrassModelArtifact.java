 ...
 *
 */
public class GrassModelArtifact extends ModelArtifact {
	...
	// Creation
	public GrassModelArtifact(String aModelPath, String aFrameName, int aWindowWidth, int aWindowHeight) {

		super("grass." + aFrameName);
		...
		if (frame.isDisplayable()) {
			...
			if (data instanceof SimulVector) {
				...
				for (final Object candidate : candidates) {
					if (candidate instanceof SheepShepherd) {
						final SheepShepherd sheep = (SheepShepherd) candidate;

						final String cmd = "get-a-sheep " + sheep.getId() + " " + sheep.getColor();

						try {
							setWith((Turtle) comp.report(cmd), sheep);
	...
	public @Nullable SimulEvent getExternalOutputEvent(String port) {
		...
		@Nullable
		AgentSet sheeps = null;

		sheeps = (AgentSet) comp.report("sheep");

		if (sheeps != null) {
			SimulVector<SheepEnergy> energies;

			energies = new ArrayedSimulVector<>();
			Iterator<Agent> iterator = sheeps.agents().iterator();
			while (iterator.hasNext()) {
				final Turtle item = (Turtle) iterator.next();
				energies = energies.appended(asSheepGrassModelInfo(item));
			}

			result = new SimulEvent(energies, eventTime);
		} else {
			result = null;
		}
	...
	// Other
	/**
	 * Set `aTarget' with `aSource'.
	 *
	 * @param aTarget
	 * @param aSource
	 * @throws AgentException
	 */
	protected void setWith(Turtle aTarget, SheepShepherd aSource) throws AgentException {
		aTarget.xandycor(aSource.getX().doubleValue(), aSource.getY().doubleValue());
		aTarget.hidden(aSource.isHidden() || aSource.isCarried());
	}
	...
	protected SheepEnergy asSheepGrassModelInfo(Turtle aTarget) {
		Double energy = (Double) aTarget.getVariable(13);
		return new SheepEnergy(aTarget.id(), energy);
	}
