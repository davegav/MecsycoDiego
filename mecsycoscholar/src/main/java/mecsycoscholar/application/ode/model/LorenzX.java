...
public class LorenzX extends Equation {
	...
	public LorenzX (double X, double A, double step) {
		super(new String[] { "X", "Y" }, new double[] { X, 0 }, new String[] { "A" }, new double[] { A }, step);
	}

	@Override
	public void dynamics () {
		double dx = (getParameters("A") * (getVariable("Y") - getVariable("X")));
		setVariable("X", (getVariable("X") + dx * time_step));
	}
