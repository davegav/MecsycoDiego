...
to setup-cars-with-speed[aspeed]
  set color black
  set lane (random 2)
  set target-lane lane
  ifelse (lane = 0) [
...
to setup-cars-with-speed-lane[aspeed alane]
  set color black
  set lane alane
  set target-lane alane
  ifelse (lane = 0) [
    setxy min-pxcor -2
  ]
  [
...
to-report mean_speed
  ifelse (any? turtles) [report mean [speed] of turtles][report 0]
end
