; The current model is based on an existing one (see info)
; C. Bourjot, B. Camus, V. Chevrier, L. Ciarletta, V. Elvinger, Y. Presse, J. Siebert, J. Vaubourg,
; Copyright (c) Université de Lorraine & INRIA, 2014-2015, v1.0
;
; The following modifications and adaptations was applied to the original model:
; - Remove globals
; - Add sheep 'energy' and 'is-carried?' atrributes
; - Move shepherd behaviour from 'go' to new procedure 'shepherd-behaviour'
; - Hide sheep with an energy equals to 0.
...
sheep-own
[
 energy					;; when comes to zero the sheep dies
 is-carried?			;; becomes true if the sheep is carried by a shepherd
]
...
to move-sheep
  ask sheep with [not hidden? and not is-carried?]
  [ wiggle
    fd sheep-speed
    if ( energy > 0 )
    [
...
to kill-em-all
  ask sheep with [not hidden? and not is-carried?]
  [
    if( energy = 0 )  ;; the sheep is not already dead nor carried
    [
      ht
    ]
  ]
...
to search-for-sheep ;; shepherds procedure
  set carried-sheep one-of sheep-here with [not hidden? and not is-carried?]
  if (carried-sheep != nobody)
    [ ask carried-sheep
        [ set is-carried? true  set color black ]			;; make the sheep invisible to other shepherds
      set color blue     					;; turn shepherd blue while carrying sheep
      fd 1 ]
end

to find-new-herd ;; shepherds procedure
  if any? sheep-here with [not hidden? and not is-carried?]
    [ set found-herd? true ]
end

to find-empty-spot ;; shepherds procedure
  if all? sheep-here [hidden? or is-carried?]
    [ ask carried-sheep
        [ set is-carried? false set color white ]                ;; make the sheep visible again
...
## ORIGINAL CREDITS AND REFERENCES
