...
; - Add cars breed
; - Remove the sample-car
; - Change road sides color to green
; - Hide cars. which leave the world.
...
to setup-road  ;; patch procedure
  ifelse (pycor < 2) and (pycor > -2) [ set pcolor grey ][set pcolor green]
end

to setup-cars
  if number-of-cars / 2 > world-width
  [
...
to separate-cars  ;; turtle procedure
  if any? other cars-here
    [ ifelse (can-move? 1) [fd 1][set xcor random-xcor]
      separate-cars ]
end
...
to go
  ...
  ask cars [
    ifelse(not hidden?)[
      let cars-ahead nobody
      ifelse patch-ahead 1 != nobody[
        set cars-ahead one-of cars-on patch-ahead 1]
      [ let y ycor
        set cars-ahead one-of turtles with [(hidden?) and (pycor = y)]]
      ifelse (cars-ahead != nobody)[
        set speed [speed] of cars-ahead
        slow-down-cars ]
      ;; otherwise, speed up
      [ speed-up-cars ]
