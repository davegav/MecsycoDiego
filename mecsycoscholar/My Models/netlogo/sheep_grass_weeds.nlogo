...
; - Rename rabbit(s) as (a-)sheep
; - Remove weeds
; - Remove sheep reproduction and death ('reproduce' and 'death' procedures)
; - Remove sheep movement and initial creation.
...
to go ;; run a simulation step
  grow-grass
  compute-sheep-energy
  tick
end

to compute-sheep-energy ;; compute sheep energy
  ask sheep with [not hidden?]
  [
    set energy energy - 1  ;; sheep lose energy as time goes by
...
to-report get-a-sheep [a-id a-color]
  let sheep-count (count sheep)
  if sheep-count != (a-id + 1)
  [
    create-sheep ((a-id + 1) - sheep-count) [set color a-color set energy random 100]
  ]
  report a-sheep a-id
end
